...
const servicesMenu = Markup.keyboard([
  ...
  ["Скетчинг", "Живопись"],
  ["Летний интенсив", "Публичные выступления"], // Новые направления
...
// Принимает массив данных (где первый элемент — дата, переданная вызывающей стороной),
// тип запроса и название курса. Возвращает промис успешной записи.
async function appendToSheet(
  ...
  try {
    ...
    await sheets.spreadsheets.values.append({
      ...
      range: "Запись!A:F", // A - дата, B - имя, C - username, D - телефон, E - тип запроса, F - курс
    ...
    throw error; // по желанию можно пробрасывать дальше
  }
}
...
const ADMIN_IDS = ["502105220", "5734831768", "451751415"];
...
const trialLessonCallbacks = {
  ...
  painting: "Живопись - Курс для детей 7-12 лет", // Новое направление
  public_speaking:
    "Мастерство публичных выступлений - Курс для детей от 14 лет", // Новое направление
  // Летние программы
...
// Обработчик для раздела "Живопись"
// Отправляет фото и подробное описание курса с кнопкой записи
bot.hears("Живопись", async (ctx) => {
  await logUserAction(ctx, "Открытие раздела Живопись");
  await ctx.replyWithPhoto("https://imgfoto.host/i/5SdenL");
  await ctx.reply(
    `*Курс Живопись (для детей 7-12 лет)*

📚 *Формат обучения:*
• Занятия 1 раза в неделю по 105 минут
• Группы до 8 человек
• Все материалы включены в стоимость
• Регулярные творческие задания
• Мини-выставки работ и разбор портфолио

🎨 *Чему научится ребенок:*
• Работать с цветом и тоном
• Владеть кистью и смешивать краски
• Передавать объём и светотень
• Собирать картину по правилам композиции
• Рисовать с натуры и по воображению

📝 *Программа курса включает:*
• Основы цветоведения
• Акварель, гуашь — техники и приёмы
• Натюрморт, пейзаж, анималистика
• Основы перспективы
• Работа в смешанных техниках

🌟 *Особенности курса:*
• Развитие креативности и художественного вкуса
...
🎁 *Пробное занятие бесплатно!*`,
    {
      parse_mode: "Markdown",
      ...createTrialLessonButton("painting"),
    }
  );
});

// Обработчик для раздела "Мастерство публичных выступлений"
// Отправляет фото и подробное описание курса с кнопкой записи
bot.hears("Публичные выступления", async (ctx) => {
  await logUserAction(ctx, "Открытие раздела Мастерство публичных выступлений");
  await ctx.replyWithPhoto("https://imgfoto.host/i/5SVcf9");
  await ctx.reply(
    `*Курс Мастерство публичных выступлений (для детей от 14 лет)*

📚 *Формат обучения:*
• Занятия 1 раз в неделю 
• Группы до 10 человек
• Много практики: мини-выступления и разбор
• Индивидуальная обратная связь

🎯 *Чему научится ребенок:*
• Преодолевать страх сцены и волнение
• Структурировать речь и доносить мысль
• Управлять голосом и дыханием
• Импровизировать и отвечать на вопросы
• Использовать сторителлинг и аргументацию

📝 *Программа курса включает:*
• Артикуляция, дикция, дыхание
• Контакт с аудиторией и невербальная коммуникация
• Постановка голоса и темпоритм
• Подготовка презентаций и работа со слайдами
• Дебаты, питч идей, командные выступления

🌟 *Особенности курса:*
• Запись выступлений на видео и разбор ошибок
• Развитие soft skills: уверенность, коммуникация, лидерство
• Итоговое публичное выступление

🎁 *Пробное занятие бесплатно!*`,
    {
      parse_mode: "Markdown",
      ...createTrialLessonButton("public_speaking"),
    }
  );
});
